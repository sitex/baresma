...
df = pd.DataFrame({
    ...
    'number': [8,4,2,1,0,2,4,8,16,32,32,16],
    'sex':['m','f','m','f','m','f','m','f','m','f','m','f'],
    'f':['ПраПраБабушка','ПраБабушка','Бабушка','Мама / Общая карма родителей','Я','Бабушка','ПраБабушка','ПраПраБабушка','ПраПраПраБабушка','ПраПраПраПраБабушка','ПраПраПраПраБабушка','ПраПраПраБабушка'],
...
def show_result():
    ...
    # is even number?
    if result % 2 == 0:
        sex_fix = 'm' if sex == 'f' else 'f'
    else:
        sex_fix = sex

    st.session_state.result = df.loc[df['sign'] == option, sex_fix].iloc[0]
